...
export default function HomePage() {
  ...
  const sendAi = async () => {
    ...
    try {
      ...
      setAiMessages(prev => [...prev, {role:"bot", text:data.response||data.answer||"Попробуйте ещё раз"}]);
  ...
  const submitContact = async (e:React.FormEvent) => {
    e.preventDefault();
    await fetch("/api/contact", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({...contactForm, source:"contact-form"})});
    setContactSent(true);
