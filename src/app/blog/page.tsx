  ...
  useEffect(() => {
    fetch("/api/blog?published=true&limit=50")
      ...
      .then(data => {
        ...
        setPosts(list);
        const cats = [...new Set(list.map((p: BlogPost) => p.category).filter(Boolean))] as string[];
        setCategories(cats);
      })
      .catch(console.error)
    ...
    <>
      ...
      <main style={{ minHeight: "100vh", background: "var(--color-bg)", paddingTop: "calc(var(--header-h) + 2rem)", paddingBottom: "4rem" }}>
        <div style={{ maxWidth: "var(--container-max)", margin: "0 auto", padding: "0 var(--container-px)" }}>
          ...
          {loading ? (
            <div style={{ textAlign: "center", color: "rgba(214,198,178,0.4)", padding: "4rem 0" }}>Загрузка...</div>
          ) : filtered.length === 0 ? (
