...
// === Lexical -> HTML converter with extended feature support ===
...
export function TextBlock({ data }: { data: TextBlockData }) {
  ...
  const imgWidth = data.imageWidth || 400
  ...
  if (hasImage) {
    const imageBlock = (
      <div
        style={{
          ...
          paddingLeft: imgPos === 'right' ? '2rem' : 0,
          paddingRight: imgPos === 'left' ? '2rem' : 0,
        }}
      >
        <img
          src={data.imageUrl}
          alt={data.imageAlt || 'Логотип Школа ПК — Первая онлайн Школа Потребительской Кооперации'}
          width={imgWidth}
          height={imgWidth}
          style={{
            width: '100%',
    ...
    return (
      <section style={{ padding: "3rem 1.5rem", background: bg }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '1rem' }}>
          {imgPos === 'left' ? <>{imageBlock}{textBlock}</> : <>{textBlock}{imageBlock}</>}
        </div>
      </section>
    )
  }
