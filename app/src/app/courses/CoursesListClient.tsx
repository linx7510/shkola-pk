...
/* Course prices mapping - can be overridden by Payload price field */
...
export default function CoursesListClient({ courses }: { courses: CourseItem[] }) {
  ...
  const getDisplayPrice = (course: CourseItem) => {
    if (course.price && course.price > 0) {
      return { amount: course.price, label: "" };
    }
    return coursePrices[course.id] || null;
  };
                ...
                <div key={course.id} className="glass-2" style={{ padding: "2rem", display: "flex", flexDirection: "column" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1rem" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: "0.8rem", color: "var(--color-text-muted)", marginTop: "0.25rem" }}>
                        {course.modulesCount} модулей · {course.lessonsCount} уроков · {course.freeLessonsCount} бесплатных
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ marginTop: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "1rem", borderTop: "1px solid var(--glass-border)" }}>
                    <div>
                      {displayPrice ? (
                        <div>
                          <span style={{ fontSize: "1.3rem", fontWeight: 800, color: "var(--color-orange-400)" }}>{displayPrice.amount.toLocaleString("ru-RU")} ₽</span>
                          {displayPrice.label && (
                            <span style={{ fontSize: "0.75rem", color: "var(--color-text-disabled)", marginLeft: "0.5rem" }}>{displayPrice.label}</span>
                          )}
                        </div>
                      ) : (
